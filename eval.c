...
Expression eval(struct ast *a)
{
    ...
    switch(a->ntype) {
        ...
        case NODE_EXP:
            ...
            break;
        case NODE_CMP:
            left = eval(((struct cmp *)a)->l);
            right = eval(((struct cmp *)a)->r);
            if (left.etype == EXP_INT && right.etype == EXP_INT) {
                if (((struct cmp *)a)->cmptype == 1) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i == right.eu.i);
                }
                if (((struct cmp *)a)->cmptype == 2) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i != right.eu.i);
                }
                if (((struct cmp *)a)->cmptype == 3) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i > right.eu.i);
                }
                if (((struct cmp *)a)->cmptype == 4) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i < right.eu.i);
                }
                if (((struct cmp *)a)->cmptype == 5) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i >= right.eu.i);
                }
                if (((struct cmp *)a)->cmptype == 6) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (left.eu.i <= right.eu.i);
                }
            } else if (left.etype == EXP_INT && right.etype == EXP_FLOAT) {
                if (((struct cmp *)a)->cmptype == 1) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (double)left.eu.i == right.eu.d;
                }
                if (((struct cmp *)a)->cmptype == 2) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = (double)left.eu.i != right.eu.d;
                ...
                if (((struct cmp *)a)->cmptype == 1) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = left.eu.d == (double)right.eu.i;
                }
                if (((struct cmp *)a)->cmptype == 2) {
                    exp.etype = EXP_BOOL;
                    exp.eu.b = left.eu.d != (double)right.eu.i;
        ...
        case NODE_LET:
            ...
            } else if (exp.etype == EXP_BOOL) {
                ((struct symasgn *)a)->sym->stype = SYMBOL_BOOL;
                ((struct symasgn *)a)->sym->su.b = exp.eu.b;
            }
            break;
        case NODE_SYMBOL:
            ...
            for(int i = 0; i < nargs; i++) {
                switch (exp[i].etype) {
                    ...
                    case EXP_STR:
                        ...
                        break;
                    case EXP_BOOL:
                        printf("%s", (exp[i].eu.b ? "true" : "false"));
                        break;
                    case EXP_NIL:
                        break;
                }
    ...
    for(i = 0; i < nargs; i++) {
        ...
        switch(s->stype) {
            ...
            case SYMBOL_STR:
                ...
                break;
            case SYMBOL_BOOL:
                oldexp[i].etype = EXP_BOOL;
                oldexp[i].eu.b = s->su.b;
                s->su.b = newexp[i].eu.b;
                break;
        }
